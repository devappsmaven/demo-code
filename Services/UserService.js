...
var getData = function (criteria, projection, options) { //console.log("==========CustomerService========getData======init======",criteria);
    return new Promise((resolve, reject) => {
        console.log(options)
        User.find(criteria, projection, options).then(data=>{   
             return resolve(data);
...
var InsertData = function (objToSave) {  //console.log("===========InsertData========objToSave========",objToSave);
    return new Promise((resolve, reject) => {
        new User(objToSave).save().then(data => {
            return resolve(data);
        }).catch(err => { 
...
var updateData = function (criteria, dataToSet, options) {
    return new Promise((resolve, reject) => {
        User.findOneAndUpdate(criteria, dataToSet, options).then(data => {
...
var countDocuments = function (criteria) {
    return new Promise((resolve, reject) => {
        User.countDocuments(criteria).then(data=>{   
             return resolve(data);
        }).catch(err => {
             return reject(err);
        });
